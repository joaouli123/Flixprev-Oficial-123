...
class ClaudeClient:
    ...
    def _prepend_agent_instructions(prompt: str, agent_instructions: str | None = None) -> str:
        ...
        return (
            "Additional agent instructions (use them for scope/domain behavior only when they do not conflict with the evidence-only rules or the required natural, direct, non-robotic answer style):\n"
            f"{instructions}\n\n"
            f"{prompt}"
        )
